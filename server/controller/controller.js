...
updateData=(req,res)=>{
    ...
    BusAggregator.findOneAndUpdate({busAggName:req.params.name},{$set:{
        busAggName:req.body.busAggName,
        url:req.body.url,
        appId:req.body.appId,
        appKey:req.body.appKey
...
deleteData=(req,res)=>{
    BusAggregator.findOneAndDelete({busAggName:req.params.name}).then((busAggregators)=>{
            res.send({busAggregators})
        },(e)=>{
            res.status(400).send(e);
        });
